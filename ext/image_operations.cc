...
#define NOTREACHED() assert(0 && "not reached");
...
void ResizeFilter::ComputeFilters(int src_size,
  ...
  std::vector<int16_t> fixed_filter_values;
  ...
  for (int dest_subset_i = dest_subset_lo; dest_subset_i < dest_subset_hi;
    ...
    assert(!filter_values.empty() && "We should always get a filter!");
...
#if (defined(OS_LINUX) && !defined(GTV)) || defined(WIN32)
...
SkBitmap ImageOperations::ResizeBasic(const SkBitmap& source,
  ...
  assert(dest.contains(dest_subset) &&
