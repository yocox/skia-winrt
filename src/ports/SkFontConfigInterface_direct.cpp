...
FcPattern* MatchFont(FcFontSet* font_set,
  ...
  FcPattern* last_match = NULL;
  for (int i = 0; i < font_set->nfont; ++i) {
    FcPattern* current = font_set->fonts[i];
    if (valid_pattern(current)) {
      FcChar8* current_family;
      if (FcPatternGetString(font_set->fonts[i], FC_FAMILY, 0, &current_family) != FcResultMatch)
        continue;

      if (!last_match &&
        strcasecmp((char *)post_config_family, (char *)current_family)) {
        last_match = current;
        continue;
      }
